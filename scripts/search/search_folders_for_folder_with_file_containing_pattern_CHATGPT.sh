...
#   4. Outputs full file paths with modification date/time, sorted newest first.
...
# ---- Step 2: Collect matching files with timestamps ----
tmpfile=$(mktemp)
for folder in "${found_folders[@]}"; do
  # Print: epoch_time formatted_date filepath
  find "$folder" -type f -name "$search_file" -printf "%T@ %TY-%Tm-%Td %TH:%TM:%TS %p\n" 2>/dev/null >> "$tmpfile"
done

# ---- Step 3: Sort newest → oldest ----
mapfile -t sorted_lines < <(sort -nr "$tmpfile")
rm -f "$tmpfile"

# ---- Step 4: Filter (if pattern given) and print ----
for line in "${sorted_lines[@]}"; do
  epoch_time=$(awk '{print $1}' <<<"$line")
  date_time=$(awk '{print $2" "$3}' <<<"$line")
  file_path=$(awk '{for (i=4; i<=NF; i++) printf $i" "; print ""}' <<<"$line" | sed 's/ *$//')
